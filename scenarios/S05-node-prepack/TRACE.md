...
`build.sh` performs the two steps separately so each execution boundary can be observed on its own, and it makes two deliberate choices that shape the evidence.
